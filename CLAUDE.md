...
Open notebooks from their folders under `MERci/notebooks/` (`before_imaging/`, `after_imaging/`, `during_imaging/`, `misc/`) in the JupyterLab file browser so that `SAMPLE_DIR` is auto-detected correctly.

## Deployment model

This repo is cloned directly into each new experiment folder (`SAMPLE_DIR`) as `SAMPLE_DIR/MERci/`. No `pip install` is needed — dependencies come from the `merci_env` conda environment. Most notebooks live two levels under the repo root (e.g. `MERci/notebooks/after_imaging/`), so they resolve paths as:
...
The `before_imaging/` notebooks live **three** levels under the repo root
(`MERci/notebooks/before_imaging/<variant>/`, where `<variant>` is
...
SAMPLE_DIR/          (the experiment root, e.g. D:\experiments\my_sample\)
  ...
  positions/         ← boundaries/manual/ (hand-drawn boundary_positions*.txt, hole*.txt),
                        boundaries/from_mosaic/ (same files, auto-derived by before_imaging/02's
                        ...
                        (from before_imaging/02's 02_create_positions_from_boundaries.ipynb)
  metadata/          ← frame_table_*.csv, shutter_sequence_*.png (before_imaging/01),
                        round_info.csv, round_bit_color_map.csv (before_imaging/03),
                        data_organization_*.csv (before_imaging/05),
                        experiment_info.yaml (before_imaging/06)
  settings/          ← hal-config-*.xml, shutter-*.xml (before_imaging/01), dave-*.xml (before_imaging/04)
  ...
  analysis/          ← thumbnails/, stats/, histograms/, mosaics/, done/
                        (produced by the after_imaging/01 and after_imaging/02 schedulers)
  merlin/            ← per-experiment MERlin config/run files (before_imaging/07):
...
notebooks/
  before_imaging/  # Pre-experiment notebooks (run in order), split into per-experiment variants:
  ...
  after_imaging/    # Online-analysis notebooks (run during the experiment)
  ...
  during_imaging/   # Live QC notebooks meant to be watched in real time WHILE HAL/Dave
                    #   is actively acquiring (as opposed to after_imaging/'s schedulers, which
                    #   run continuously in the background, or its one-shot review notebooks)
    stage_z_drift.ipynb                            # one-shot QC (moved from after_imaging/08_stage_z_drift.ipynb --
    ...
    imaged_fovs.ipynb                              # live acquisition-progress map: reuses before_imaging/
    ...
    round_mosaics.ipynb                            # live quick-look mosaic, one state table + one loop
                                                   ...
                                                   #   this quick-look tool and after_imaging/02_round_scheduler.
  ...
  tests/           # Diagnostic/recovery notebooks built to investigate or fix ONE specific
                    ...
                    #   numbered before_imaging pipeline or a standing analysis tool. Two
                    #   levels deep like after_imaging/misc/during_imaging (MERCI_DIR = ...parent.parent).
...
data/
  configs/
    hal/            # hal-config-{mic}.xml — HAL config templates (one per microscope)
      mosaic_helper/ # hand-crafted per-microscope 10x/60x mosaic-tool setup configs (HAL + shutter
                     ...
                     #   auto-detection glob in before_imaging/01 (hal-config-*.xml directly under
    ...
    merlin/         # shared MERlin reference files, copied from R:\Software\merfish-parameters\
                    #   (2026-active files only; see before_imaging/07 + merlin_config.py):
  ...
  readouts.csv      # default codebook readout table (bit number -> readout name), read by before_imaging/05
```
...
Run the eight `before_imaging/<variant>/` notebooks (variant = `reference`) in
...
(`before_imaging/tumor/{epi,disk}/`, `before_imaging/lineage_tracing/{merfish,lineage}/`);
...
`notebooks/before_imaging/lineage_tracing/merfish_multi_z/README.md` and the
...
- HAL config templates — `data/configs/hal/hal-config-{mic}.xml`; auto-detected in `before_imaging/01` by microscope name; patched by `create_hal_config` (sets frames, shutters, z_offsets, filetype, exposure_time)
- Kilroy config files — `data/configs/kilroy/kilroy-config-*-{mic}-*-{YYMMDD}.xml`; resolved in `before_imaging/04` by `find_kilroy_config` (newest by YYMMDD; falls back to MF2 when the microscope has no config), copied to `SAMPLE_DIR/settings/`, and used as the source of fluidic protocol names for the Dave recipe. Protocol names are **not** standardised across microscopes (e.g. `Cleave Adaptors` vs `Cleave Adaptor`), so `KilroyProtocolResolver` token-matches each logical dave step (cleave / hybridize k / readouts / image buffer) to the real `<protocol>` name in the chosen config.
