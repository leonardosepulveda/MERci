...
geometry: `MFX`/`ST2` = 2304×2304, `MF2`–`MF5` = 2048×2048
(`get_camera_frame_size`/`get_fov_geometry`). Pixel size is read from each
scope's MERlin microscope JSON (`data/configs/merlin/microscope/`) by
`get_camera_pixel_size_um`. Never hard-code it in a notebook. Acquisition type (orthogonal to the above):
