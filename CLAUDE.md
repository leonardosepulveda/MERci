...
src/MERci/
  common/
    ...
    experiment_info.py  # ExperimentInfo (core fields + extra dict, mirrors SeriesInfo.extra_meta's
                    ...
                    #   IS the id). Used only where the true id is actually needed (notebook 06's
                    #   DATA_HOME/MERLIN_HOME/FOLDER_NAME, notebook 07's resolve_cluster_sample_dir) --
                    #   NOT for the per-notebook local file-naming convention (positions_*.txt,
                    #   dave-*.xml, etc.), which stays SAMPLE_DIR.name as before (see notebooks 06/07's
                    #   LOCAL_NAME vs SAMPLE_NAME below)
  acquisition/
