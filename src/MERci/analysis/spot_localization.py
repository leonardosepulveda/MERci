...
**Visualisation**
    ...
    (the max-projection figure is ``plots.spot_localization_plots.plot_max_projections``)
"""
...
def __getattr__(name):
    # Plotting moved to MERci.plots.spot_localization_plots; keep old imports working
    # (e.g. notebooks already exported into an experiment folder).
    if name in ('plot_max_projections',):
        import importlib
        return getattr(importlib.import_module('MERci.plots.spot_localization_plots'), name)
