# prepare_imaging / tumor / epi

Pre-experiment notebooks for the **epifluorescence** acquisition of a tumor
experiment: a **single tissue section** per coverslip. One of the two
acquisition types under `tumor/`; the other is `../disk/` (spinning-disk
confocal).

Duplicated from the shared `tumor` notebook set — tune the parameters here for
the epi acquisition. Keep the notebook *logic* in sync with `../../reference/`;
change only the experiment parameters here.

These notebooks live **four** levels under the repo root
(`MERci/notebooks/prepare_imaging/tumor/epi/`), so they resolve
...
## Parameters to set for a tumor epi run
