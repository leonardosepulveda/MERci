# before_imaging / multi_z

Pre-experiment notebooks for a **variable-z-per-FOV** MERFISH acquisition of a
lineage-tracing experiment. A variant of `../lineage_tracing/merfish/` for
tissue whose depth varies across the coverslip: instead of imaging every FOV
to one fixed z-depth (wasting time/disk on thin regions), this workflow
images a full-depth DAPI (cells) round first, measures each FOV's real tissue
thickness from it, buckets FOVs into a handful of z-depth tiers, and images
each bits round to only the depth its tier actually needs.
...
between them). These notebooks live **three** levels under the repo root
(`MERci/notebooks/before_imaging/multi_z/`), so they resolve
`MERCI_DIR = Path(os.getcwd()).parent.parent.parent`.

## Why a separate notebook set instead of a flag on `../lineage_tracing/merfish/`
...
in sync with `../lineage_tracing/merfish/` wherever the two share a step
...
rather than here -- see that notebook's own docstring.

| # | Notebook | vs. `../lineage_tracing/merfish/` |
|---|----------|--------------------|
...
| 08 | `08_create_experiment_info.ipynb` | Renumbered from `merfish`'s `06_create_experiment_info.ipynb`. Bits HAL config lookup (for exposure time) checks both `settings/` and `multi_z/`. |
| 09 | `09_create_merlin_scripts.ipynb` | Renumbered from `merfish`'s `07_create_merlin_scripts.ipynb`. Passes `--allow-ragged-z-stacks` to the generated slurm submit script whenever `round_info.csv` has any `tissue_thickness="multi"` row. |

## The `multi_z/` folder

Notebook 04 writes each tier's HAL config **and** shutter XML into one combined
...
Notebook 04 rewrites each positions file in place, adding a 3rd column: the
...
it requires the patched `v2Generator.py` in `../../../misc/dave_multi_z/`
(**outside** the MERci repo; see that folder's own `README.md` for what it
changes and how to deploy it to the microscope computer). Both halves are
required together: this patch alone (without the `dave.py` changes in this
repo) or this repo's changes alone (without the patch) will not produce a
working per-FOV z recipe.

## Expected layout

Same boundary-file conventions as `../lineage_tracing/merfish/`:
...
- `04`: `N_TIERS`, `MICROSCOPE`, `POWER`, `color_seq` for bits, `EXPOSURE_TIME`
  ...
  bits sweep.
- `05`: `round_bit_color` mapping to match the codebook (derives `N_HYBS`).
- `06`: `USE_ADAPTORS`, `FIRST_HYB_NO_CLEAVE`, `INCLUDE_FINAL_CLEAVE`.
- `07`: Note: multi-tissue MERlin analysis is per tissue / per boundary —
  confirm the intended workflow before relying on the generated
  data-organization.
- `09`: MERlin's ragged-z-stack decoding is still incomplete upstream as of
